...
\section{Pipeline properties}
    ...
    times of the single-precision floating-points multiplication. Figure~\ref{fig:fp_prediction_512}
    ...
    first core group and 17 to 32 is the second one.) $t_{j,k}$ is the thread number $k$ of the
    warp $j$. 
        \begin{figure}[h]
      \centering
   ...
   \end{figure}
    ...
    group core is provided in integer multiplication but the second. $t_{j,k}$ is the thread number
    $k$ of the warp $j$.
   \begin{figure}[h]
      \centering
       \begin{tabular}{ | r || c | c | c | c || c | c | c | }
