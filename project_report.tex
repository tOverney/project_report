...
\section{Goals}
    ...
    when used for cryptography and applications using large integers arithmetic in order to suggest
...
\chapter{First approach}
    ...
    a special kind of modular multiplication. Implementing such an algorithm in hardware can
    be done in many ways, one of the most efficient is the rolled implementation described
    by Algorithm~\ref{alg:montgomery}. Its operands $X$ and $Y$ are already prepared in software for the montgomery multiplication.
...
\chapter{Fermi's architecture}
    ...
    \section{Basics of Fermi's scheduling}
        ...
        core, every benchmarking experiment was done using a grid size of one.
...
\section{Pipeline properties}
    ...
    The flat line for the floating point pipeline means that up to 576 threads fill the pipeline and almost finish at
    the same time (few cycles difference between threads).
...
\section{Mixing float and integer multiplication}
    ...
    integer multiplication able to run in parallel on an SM was only half the number of 
