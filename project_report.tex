...
\chapter{First approach}
    ...
    %TODO pourquoi on a pas fait ça.
...
\chapter{Fermi's architecture}
    ...
    with 16 of these SMs. %TODO dire au debut benchmark
    \begin{figure}[H]
    \centering
...
\section{Pipeline properties}
    ...
    multiplication than for the float multiplication, as GPUs are optimized for float computation
    and the performance should decrease faster when increasing the number of parallel threads running for the integer
    than the float as the throughput for the integer multiplication is indicated to be only half the float multiplication's
    in the CUDA Programming Guide~\cite{cudaprog}. For both operations, it is expected that after the deterioration has started the degradation rate will be the same. 
    ...
    and it shows that the integer multiplication times start to grow at 289 threads in the block against
    577 for the float multiplications.
    
    The flat line for the floating point pipeline means that up to 576 threads fill the pipeline and almost finish simultaneously (few cycles difference between threads).
    Since we have 32 cores, then we compute the pipeline depth as $\cfrac{576}{32} = 18$  stages. 
...
\chapter{Benchmarking program}
   \lstinputlisting[language=C]{integer\_multiplication\_benchmark.cu} 
